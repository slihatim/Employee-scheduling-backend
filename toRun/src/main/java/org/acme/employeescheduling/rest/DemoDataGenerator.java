...
public class DemoDataGenerator {
    ...
    public EmployeeSchedule generateDemoData(DemoDataParameters parameters) {
        ...
        List<ShiftAssignment> shiftAssignments = new LinkedList<>();
        for (int i = 0; i < parameters.daysInSchedule; i++) {
            ...
            shiftAssignments.addAll(generateShiftsForDay(parameters, date, random));
        }
        AtomicInteger countShift = new AtomicInteger();
        shiftAssignments.forEach(s -> s.setId(Integer.toString(countShift.getAndIncrement())));
        employeeSchedule.setShifts(shiftAssignments);

        return employeeSchedule;
    }

    private List<ShiftAssignment> generateShiftsForDay(DemoDataParameters parameters, LocalDate date, Random random) {
        List<ShiftAssignment> shiftAssignments = new LinkedList<>();
        for (String location : parameters.locations) {
            ...
            for (LocalTime shiftStartTime : shiftStartTimes) {
                ...
                shiftAssignments.addAll(generateShiftForTimeslot(parameters, shiftStartDateTime, shiftEndDateTime, location, random));
            }
        }
        return shiftAssignments;
    }

    private List<ShiftAssignment> generateShiftForTimeslot(DemoDataParameters parameters,
        ...
        for (int i = 0; i < shiftCount; i++) {
            ...
            shiftAssignments.add(new ShiftAssignment(timeslotStart, timeslotEnd, location, requiredSkill));
        }
        return shiftAssignments;
    }
